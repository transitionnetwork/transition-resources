...
abstract class Page_Base {
	 ...
	 *
	 * Either "settings_page_" or "admin_page_". Default "settings_page_".
	 *
	 * @since 1.0.0
	 * @var string
	 */
	...
	public function admin_menu( $page_handle = '', $page_slug = '' ) {
		...
		// Attach to Settings menu item unless Sub-page.
		if ( empty( $page_slug ) ) {
	...
	protected function admin_help_get() {
		...
		if ( ! file_exists( $template ) ) {
			$template = TRANSITION_RESOURCES_PATH . $this->path_template . $this->path_help . 'page-settings-help.php';
		}
	...
	public function page_render() {
		...
		if ( ! file_exists( $template ) ) {
			$template = TRANSITION_RESOURCES_PATH . $this->path_template . $this->path_page . $template_name;
		}
	...
	public function meta_box_submit_render( $unused, $metabox ) {
		...
		if ( ! file_exists( $template ) ) {
			$template = TRANSITION_RESOURCES_PATH . $this->path_template . $this->path_metabox . 'metabox-settings-submit.php';
		}
	...
	public function form_submitted() {
		 ...
		 * @param string $submit_id The Settings Page form submit ID.
		 */
		do_action( $this->hook_prefix . '/settings/form/save/before', $submit_id );
		 ...
		 * @param string $submit_id The Settings Page form submit ID.
		 */
		do_action( $this->hook_prefix . '/settings/form/save/after', $submit_id );
